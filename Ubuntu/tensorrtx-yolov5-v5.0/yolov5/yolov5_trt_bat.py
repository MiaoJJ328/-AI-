...
if __name__ == "__main__":
    ...
    # 初始化摄像头
    cap = cv2.VideoCapture(0, cv2.CAP_V4L2)  # 0 表示默认摄像头
    cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('M','J','P','G'))
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
    print("实际格式:", cap.get(cv2.CAP_PROP_FOURCC))  # 应输出 1196444237（MJPG的整型值）
    print("实际分辨率:", cap.get(cv2.CAP_PROP_FRAME_WIDTH), "x", cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    if not cap.isOpened():
    ...
    try:
        while True:
            ...
            # 显示处理后的帧
            cv2.imshow("Camera Detection", batch_image_raw[0])
