...
class CatPaw:
    def __init__(self):
        ...
        ################################################
        # 新增发送控制相关变量
        self.send_flag = True  # True:发送cat_detection, False:发送qr_code_detection
        ...
        while self.sending_active and not rospy.is_shutdown():
            try:
                with self.socket_lock:
                    # 初始化socket（如果未创建或已关闭）
                    if not hasattr(self, 's') or (self.s is None) or (self.s.fileno() == -1):  # 使用fileno检查有效性
                        self.s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                        self.s.connect((HOST, PORT))
                        rospy.loginfo("成功连接到服务端: %s:%d" % (HOST, PORT))
                    ...
                    rospy.loginfo("已发送检测状态: %s" % msg)
                
                time.sleep(0.5)  # 控制发送频率
                
            except Exception as e:
                rospy.logerr("发送错误: %s" % str(e))
                time.sleep(1)
                try:
                    if hasattr(self, 's') and self.s is not None:
                        self.s.close()
                        self.s = None  # 重置socket对象
                except:
                    pass
    ...
    def shutdown(self):
        """节点退出时安全终止所有线程和资源（修改部分）"""
        rospy.loginfo("节点关闭中，清理资源...")
        self._shutdown_flag = True
        self.stop_sending()  # 新增停止发送线程
        self.stop_clean_task()
        # 强制停止机器人运动
        self.vel_cmd.linear.x = 0.0
        self.vel_cmd.angular.z = 0.0
