...
void OV2640::run(void)//获取一个新的帧缓冲区，如果之前有缓冲区，则将其返回给驱动程序以供重用。
{
    if (fb)
...
void OV2640::runIfNeeded(void)//如果当前没有帧缓冲区，则调用run()方法。
{
    if (!fb)
        run();
}

int OV2640::getWidth(void)//返回当前帧的宽度。
{
    runIfNeeded();
    return fb->width;
}

int OV2640::getHeight(void)//返回当前帧的高度。
{
    runIfNeeded();
    return fb->height;
}

size_t OV2640::getSize(void)// 返回当前帧的大小。
{
    runIfNeeded();
    if (!fb)
...
uint8_t *OV2640::getfb(void)//返回指向当前帧数据的指针
{
    runIfNeeded();
    if (!fb)
...
framesize_t OV2640::getFrameSize(void)//返回当前的帧大小设置。
{
    return _cam_config.frame_size;
}

void OV2640::setFrameSize(framesize_t size)//设置帧大小。
{
    _cam_config.frame_size = size;
}

pixformat_t OV2640::getPixelFormat(void)//返回当前的像素格式设置。
{
    return _cam_config.pixel_format;
}

void OV2640::setPixelFormat(pixformat_t format)//设置像素格式。
{
    switch (format)
    {
...
esp_err_t OV2640::init(camera_config_t config)//初始化摄像头，使用提供的配置来设置摄像头参数，并检查初始化是否成功。

{
    ...
    // ESP_ERROR_CHECK(gpio_install_isr_service(0));安装GPIO中断服务，并检查是否有错误发生。
