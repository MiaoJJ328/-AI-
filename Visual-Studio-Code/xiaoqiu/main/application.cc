...
#include <sys/time.h>
#include <esp_http_server.h>  // 用于URL编码函数
...
void Application::SynthesizeAndPlaySpeech(const std::string& text) {
    // 1. 对文本进行URL编码
    std::string encoded_text = UriEncode(text);

    // 2. 生成时间戳和随机数
    struct timeval tv;
    gettimeofday(&tv, NULL);
    std::string timestamp = std::to_string(tv.tv_sec * 1000 + tv.tv_usec / 1000);
    std::string nonce = std::to_string(esp_random());

    // 3. 构建签名参数
    // std::stringstream canonicalized_query;
    // canonicalized_query << "appkey=Ok3AlAfU52fxSSpl"
    //                   << "&format=wav"
    //                   << "&sample_rate=16000"
    //                   << "&text=" << encoded_text
    //                   << "&timestamp=" << timestamp
    ...
    mbedtls_md_hmac_starts(&ctx, 
        reinterpret_cast<const unsigned char*>(ALIYUN_ACCESS_KEY_SECRET), 
        strlen(ALIYUN_ACCESS_KEY_SECRET));
    mbedtls_md_hmac_update(&ctx, 
        reinterpret_cast<const unsigned char*>(canonicalized_query.str().c_str()), 
        canonicalized_query.str().length());
    ...
    esp_http_client_config_t config = {
       ...
       .transport_type = HTTP_TRANSPORT_OVER_SSL  // 必须使用HTTPS
    };
    ...
    std::string auth_header = 
        "Bearer " + std::string(ALIYUN_ACCESS_KEY_ID) + ":" + hmac_hex;
    esp_http_client_set_header(client, "Content-Type", "application/x-www-form-urlencoded");
    esp_http_client_set_header(client, "Authorization", auth_header.c_str());

    // 7. 设置POST数据（参数放在Body中）
    std::string post_data = canonicalized_query.str();
    esp_http_client_set_post_field(client, post_data.c_str(), post_data.length());

    // 8. 执行请求
    ESP_LOGI(TAG, "Request Data: %s", post_data.c_str());
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (text != NULL) {
                ...
                // 新增指令检测-开启食物门
                if (message.find("开始食物门") != std::string::npos) {
                    ESP_LOGI(TAG, "[指令检测] 用户发出了开始食物门指令");
                ...
                // 新增指令检测-关闭食物门
                if (message.find("关闭食物门") != std::string::npos) {
                    ESP_LOGI(TAG, "[指令检测] 用户发出了关闭食物门指令");
                ...
                // 新增指令检测-玩具顺时针旋转
                if (message.find("玩具顺时针旋转") != std::string::npos) {
                    ESP_LOGI(TAG, "[指令检测] 用户发出了玩具顺时针旋转指令");
                    const char* payload = "{\"clockwise\":1}";
                    int msg_id = esp_mqtt_client_publish(ali_mqtt_client, AliyunPublishTopic_user_update, payload, strlen(payload), 1, 0);
                    ESP_LOGI(TAG, "Sent auto_clean command, msg_id=%d", msg_id);
                }

                // 新增指令检测-玩具逆时针旋转
                if (message.find("玩具逆时针旋转") != std::string::npos) {
                    ESP_LOGI(TAG, "[指令检测] 用户发出了玩具逆时针旋转指令");
                    const char* payload = "{\"clockwise\":0}";
                    int msg_id = esp_mqtt_client_publish(ali_mqtt_client, AliyunPublishTopic_user_update, payload, strlen(payload), 1, 0);
                    ESP_LOGI(TAG, "Sent auto_clean command, msg_id=%d", msg_id);
                }

                // 新增指令检测-玩具逆时针旋转
                if (message.find("关闭玩具") != std::string::npos) {
                    ESP_LOGI(TAG, "[指令检测] 用户发出了关闭玩具指令");
                    const char* payload = "{\"play\":0}";
                    int msg_id = esp_mqtt_client_publish(ali_mqtt_client, AliyunPublishTopic_user_update, payload, strlen(payload), 1, 0);
                    ESP_LOGI(TAG, "Sent auto_clean command, msg_id=%d", msg_id);
                }

                // 新增指令检测-定时投喂
                if (message.find("设置定时喂食1分钟一次最大重量20g") != std::string::npos) {
                    ESP_LOGI(TAG, "[指令检测] 用户发出了设置定时喂食1分钟一次最大重量20g指令");
                    const char* payload = "{\"auto_feed\":1,\"timeyear\":2025,\"timemonth\":4,\"timedate\":15,\"timehours\":23,\"timeminutes\":59,\"timeseconds\":54,\"minutesintotal\":1,\"maxweight\":20}";
                    int msg_id = esp_mqtt_client_publish(ali_mqtt_client, AliyunPublishTopic_user_update, payload, strlen(payload), 1, 0);
                    ESP_LOGI(TAG, "Sent auto_clean command, msg_id=%d", msg_id);
                }
